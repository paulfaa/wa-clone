...
describe('ChatViewComponent', () => {
    ...
    describe('ngOnInit', () => {
        it('calls messageService.$getFilteredMessages with the first date of the yearMonthMap', () => {
            ...
            expect(
                mockMessageService.$getFilteredMessages
            ).toHaveBeenCalledWith(firstYearMonth)
        })
    })
    ...
    describe('toggleFavourite', () => {
        ...
        beforeEach(() => {
            ...
            mockFavouritesService['favouritesMap'] = new Map<
                string,
        ...
        it('removes a message from favourites if it was already favourited', () => {
            //arrange
            mockFavouritesService.isFavourite = jasmine
                .createSpy()
                .and.returnValue(true)
            expect(
                mockFavouritesService['favouritesMap'].has(
                    favouritedMessage.id!
                )
            ).toBeTrue
            ...
            //assert
            expect(
                mockFavouritesService.removeFromFavourites
            ).toHaveBeenCalledWith(favouritedMessage)
            expect(
                mockFavouritesService['favouritesMap'].has(
                    favouritedMessage.id!
                )
        ...
        it('adds a message to favourites if it is not favourited', () => {
            ...
            expect(mockFavouritesService['favouritesMap'].has(newMessage.id!))
                .toBeFalse
            ...
            //assert
            expect(mockFavouritesService.addToFavourites).toHaveBeenCalledWith(
                newMessage
            )
            expect(mockFavouritesService['favouritesMap'].has(newMessage.id!))
                .toBeTrue
    ...
    describe('page', () => {
        it('displays the message objects contained in the array', () => {
            ...
            // Act
            component.ngOnInit()
            const messageCount1 = fixture.debugElement.queryAll(
                By.css('.messageBubble')
            ).length
